...
user = st.sidebar.text_input("Enter User", value="root")
...
if st.session_state["connection"]:
    ...
    database = st.sidebar.selectbox("Select Existing Database", ["No Database Selected"] + databases)
    ...
    new_database = st.sidebar.text_input("Create and Select New Database")
...
tab1, tab2 = st.tabs(["Query Execution", "Messages"])

with tab1:
    if st.session_state["db_connection"]:
        query = st.text_area("Enter SQL Query", height=165)
        col1, col2, col3, col4 = st.columns([0.55, 0.60, 2, 1])
        with col4:
